...
py C:/workspace/bi/jobs/my_etl_08/tables_validation.py
...
py ./jobs/my_etl_08/tables_validation.py

python3 ./jobs/my_etl_08/dataset_validation.py ppltx-bi-developer --etl-name dataset --etl-action daily --days-back 2
...
for dataset_name, tables_conf in etl_configuration.items():
    ...
    tables_threshold = tables_conf["tables"]
    exclude_list = tables_conf["exclude"]

    for table in tables:
        table_name = table.table_id

        if table_name in exclude_list:
            print(f"{table_name} in exclude list")
            continue
        ...
        tables_dict["table_name"].append(table_obj.table_id)
        tables_dict["dataset"].append(dataset_name)
        table_modified = table_obj.modified
        tables_dict["last_update"].append(table_modified.strftime("%Y-%m-%d %H"))
        tables_dict["rows"].append(table_obj.num_rows)
        tables_dict["bytes"].append(table_obj.num_bytes)
        ...
        tables_dict["alert_threshold"].append(tables_threshold.get(table_name, 24))
...
if not df_alerts.empty:
    ...
    writeFile(logs_path / "dataset_alert_slack_msg.md", msg)
    print(msg)
