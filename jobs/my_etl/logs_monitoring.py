...
python3 ./bi/jobs/my_etl/logs_monitoring.py  bi-course-461012 --etl-name log --etl-action daily
...
py ./jobs/my_etl_08/logs_monitoring.py

python3 ./jobs/my_etl_08/logs_monitoring.py  ppltx-bi-developer --etl-name log --etl-action daily --days-back 2


python3 ./jobs/my_etl_08/logs_monitoring.py  ppltx-bi-developer --etl-name log --etl-action daily --days-back 2

bi-course-461012 --etl-name animals --etl-action daily --days-back 2
...
for alert_group_name, alerts in etl_configuration.items():
    ...
    query_sql = readFile(home / repo_name / repo_tail / f"queries/log_alert.sql")
    conf = alerts["alerts"]
    # print(conf)
...
if (df_all[alert_columns]).any():
    ...
    writeFile(logs_path /  "log_monitoring_msg.md", msg)
    print(msg)
